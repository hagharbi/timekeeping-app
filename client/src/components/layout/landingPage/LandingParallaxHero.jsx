...
const LandingParallaxHero = ({ yOffset }) => (
  <Container >
      ...
      { createParallaxUp    (     CommentsParallax,      yOffset,  4,5  )}
      { createParallaxUp    (     Snowhill0,             yOffset,  7.5 )}
      { createParallaxDown  (     CloudsUp,              yOffset,  0  )}
      { createParallaxDown  (     Clouds,                yOffset,  0  )}
      { createParallaxUp    (     Snowhill4,             yOffset, 6  )}
      { createParallaxDown  (     GearsParallax,         yOffset,  0  )}

      { createParallaxUp    (     StarsParallax,         yOffset,  3  )}
      { createParallaxUp    (     Snowhill3,             yOffset, 5.5  )}
      { createParallaxUp    (     ClockhaloRingParallax, yOffset,  1.8)}
      { createParallaxUp    (     ClockParallax,         yOffset,  1.8)}

      { createParallaxRight (     EnvilopeParallax,      yOffset,  1  )}
      { createParallaxUp    (     Snowhill2,             yOffset,  5  )}
      { createParallaxUp    (     Snowhill2,             yOffset,  4  )}
      { createParallaxUp    (     CalendarParallax,      yOffset,  3.5  )}
      { createParallaxUp    (     Snowhill1,             yOffset,  3.5  )}
